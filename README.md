...
Add quick-repls.el to your init.el via e.g. (load-file "~/.emacs.d/quick-repls.el"). 
...
M-x quick-repls-connect

Wait for about 1 minute and check *Messages* buffer for "Clj connection buffer: .\* Cljs connection buffer .\*". 

Do not attempt to execute quick-repls-connect twice, if you need to restart run cider-quit and then quick-repls-connect. 
...
If you would like to be able to switch to the relevant repl buffer based on if you are currently in a clj or cljs buffer then you need to set the custom variable cider-switch-to-repl-command to quick-repls-switch-to-relevant-repl 
...
```
    '(cider-switch-to-repl-command (quote quick-repls-switch-to-relevant-repl))
```
